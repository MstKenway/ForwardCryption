...
struct field_s {
  ...
  int (*is1)(element_ptr);
  int (*is0)(element_ptr);
  int (*sign)(element_ptr);  // satisfies sign(s) = -sign(-s)
...
Whitespace is ignored. Points have the form "['s,y']" or "'O'",
...
/*@manual epow
Set 's' = 'a'^'n'^, where 'n' is an element of a ring *Z*~N~
for some 'N' (typically the order of the algebraic structure 's' lies in).
*/
...
Writes the s-coordinate of 'e' to the buffer 'data'
*/
int element_to_bytes_x_only(unsigned char *data, element_t e);
/*@manual etrade
Assumes 'e' is a point on an elliptic curve.
Sets 'e' to a point with
s-coordinate represented by the buffer 'data'. This is not unique.
For each 's'-coordinate, there exist two different points, at least
...
/*@manual etrade
Assumes 'e' is a point on an elliptic curve.
Returns the length in bytes needed to hold the s-coordinate of 'e'.
*/
...
For polynomials, returns coefficient of 's^n^'.
...
/*@manual epow
Computes 's' such that 'g^s^ = h' by brute force, where
's' lies in a field where `element_set_mpz()` makes sense.
*/
void element_dlog_brute_force(element_t x, element_t g, element_t h);

/*@manual epow
Computes 's' such that 'g^s^ = h' using Pollard rho method, where
's' lies in a field where `element_set_mpz()` makes sense.
*/
